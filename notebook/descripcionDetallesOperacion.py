...
def describir_datos(data_frame_limpio):
    ...
    print(f"estadisticas{data_frame_limpio[['costo_semestre']].describe()}")
    print(f"valores categoricos{data_frame_limpio['id_programa'].value_counts()}")
    print(f"fecha minima{data_frame_limpio['fechaActualización'].min()}")
    print(f"fecha maxima{data_frame_limpio['fechaActualización'].max()}")